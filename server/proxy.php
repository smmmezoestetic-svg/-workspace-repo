...
// ── Конфигурация ────────────────────────────────────────────────────────────
// Прокси лежит в <корень сайта>/skin-test-app/api/, поэтому «выше корня» —
// это три уровня вверх. Порядок задаёт приоритет: сначала место вне зоны
// доступа веб-сервера, потом корень сайта, потом соседний файл под .htaccess.
$configCandidates = [
    __DIR__ . '/../../../skin-test-config.php', // выше корня сайта — предпочтительно
    __DIR__ . '/../../skin-test-config.php',    // корень сайта
    __DIR__ . '/config.php',                    // рядом с proxy.php, закрыт .htaccess
];
$config = [];
